...
class E2E_v6_loss(nn.Module):
    def __init__(self, device, qtl, size_average=True):
        ...
        self.device = device
        self.size_average = size_average
        self.lmd_1 = -0.25 # Scalared value. Up to user's choice. Disguised due to confidentiality.
        self.lmd_2 = -0.2 # Scalared value. Up to user's choice. Disguised due to confidentiality.
    ...
    def forward(self, vlt_o, vlt_t, sf_o, sf_t, out, target):
        ...
        self.q_loss = QauntileLoss(out, target) #LOSS_FUN1 up to user's choice. Can be self-defined or nn.MSELoss, QauntileLoss, etc.
        self.vlt_loss = QauntileLoss(vlt_o, vlt_t) #LOSS_FUN2 up to user's choice. Can be self-defined or nn.MSELoss, QauntileLoss, etc.
        self.sf_loss = QauntileLoss(sf_o, sf_t) # Loss_FUN3 should be quantile losses summed over each quantile
        self.loss = self.quantile_loss + self.lmd_1 * self.vlt_loss + self.lmd_2 * self.sf_loss

        return self.quantile_loss, self.loss
