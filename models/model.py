...
class Decoder_MLP(nn.Module):
    ...
    def init_weights(self):
        """Initialize weights."""
        # Gaussian initialization for linear layers
        nn.init.kaiming_normal_(self.global_mlp.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.global_mlp.bias, 0)
        
        nn.init.kaiming_normal_(self.local_mlp.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.local_mlp.bias, 0)
        
        nn.init.kaiming_normal_(self.bridge_mlp.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.bridge_mlp.bias, 0)
...
class MQ_RNN(nn.Module):
    ...
    def init_weights(self):
        """Initialize weights."""
        # Gaussian initialization for linear layers
        nn.init.kaiming_normal_(self.linear_encoder.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.linear_encoder.bias, 0)
        ...
        # Initialize Decoder_MLP weights
...
class End2End_v6_tc(nn.Module):
    ...
    def __init__(self, device, tf_graph=False):
        ...
        self.hidden_dim = 50 # Disguised dimension sizes. Up to user's choice.
        self.output_dim = 1

        # Disguised initialization for fully connected layers, due to confidential dimension sizes.
        self.fc_vlt_1 = nn.Linear(self.vlt_dim,50)
        self.fc_vlt_2 = nn.Linear(50,20)
        self.fc_vlt_3 = nn.Linear(20,self.vlt_dim)
        
        self.fc_3 = nn.Linear(self.vlt_dim,50)
        self.fc_4 = nn.Linear(100,100)
        self.fc_5 = nn.Linear(100,self.output_dim)
    ...
    def init_weights(self):
        ...
        nn.init.kaiming_normal_(self.fc_vlt_1.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.fc_vlt_1.bias, 0)
        
        nn.init.kaiming_normal_(self.fc_vlt_2.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.fc_vlt_2.bias, 0)
        
        nn.init.kaiming_normal_(self.fc_vlt_3.weight, mode='fan_out', nonlinearity='linear')
        nn.init.constant_(self.fc_vlt_3.bias, 0)
        
        nn.init.kaiming_normal_(self.fc_3.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.fc_3.bias, 0)
        
        nn.init.kaiming_normal_(self.fc_4.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.fc_4.bias, 0)
        
        nn.init.kaiming_normal_(self.fc_5.weight, mode='fan_out', nonlinearity='linear')
        nn.init.constant_(self.fc_5.bias, 0)

        # Initialize MQ_RNN weights
        self.sf_mqrnn.init_weights()
